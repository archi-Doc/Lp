...
public class NetsphereGeneratorV2 : IIncrementalGenerator, IGeneratorInformation
{
    ...
    private void Emit(SourceProductionContext context, (Compilation Compilation, ImmutableArray<TypeDeclarationSyntax?> Types) source)
    {
        ...
        var netsphereGeneratorOptionAttributeSymbol = compilation.GetTypeByMetadataName(NetsphereGeneratorOptionAttributeMock.FullName);
...
#pragma warning restore RS1024 // Symbols should be compared for equality
        ...
        foreach (var x in source.Types)
        {
                ...
                !processed.Contains(s))
            {
                ...
                {
                    if (SymbolEqualityComparer.Default.Equals(y.AttributeClass, netServiceInterfaceAttributeSymbol))
                    { // NetServiceInterface
                        body.Add(s);
                        break;
                    }
                    ...
                    {// NetsphereGeneratorOption
                        ...
                        var va = new VisceralAttribute(NetsphereGeneratorOptionAttributeMock.FullName, y);
                        var ta = NetsphereGeneratorOptionAttributeMock.FromArray(va.ConstructorArguments, va.NamedArguments);
